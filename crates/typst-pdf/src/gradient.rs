...
use pdf_writer::{Content, Filter, Finish, Rect, Ref};
use typst::geom::{
...
fn shading_function_opacity(ctx: &mut PdfContext, ref_: Ref, stops: &[(Color, Ratio)]) {
    ...
    for window in stops.windows(2) {
        ...
        // Get the alpha channels of the two stops.
        let first_col = Color::from(first.0.alpha_as_luma());
...
/// Writes a single Coons Patch as defined in the PDF specification
/// to a binary vec. This is a D65 Gray variant of `write_patch`
///
/// Structure:
///  - flag: `u8`
///  - points: `[u16; 24]`
///  - colors: `[u16; 4]`
fn write_patch_grayscale(
    target: &mut Vec<u8>,
    t: f32,
    t1: f32,
    c0: u16,
    ...
    let colors = [c0.to_be(), c0.to_be(), c1.to_be(), c1.to_be()];
...
fn compute_vertex_stream_grayscale(
    ...
    for window in conic.stops.windows(2) {
        ...
        let c0 = Color::from(c0.alpha_as_luma());
        let c1 = Color::from(c1.alpha_as_luma());
        ...
        while t_x < t1.get() {
            ...
            write_patch_grayscale(
                &mut vertices,
                t_x as f32,
                t_next as f32,
                ColorSpace::D65Gray.convert(c)[0],
                ColorSpace::D65Gray.convert(c_next)[0],
                angle,
            );
