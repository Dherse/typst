use comemo::Tracked;
use typst_library::diag::{HintedStrResult, SourceResult};
use typst_library::engine::Engine;
use typst_library::foundations::{
    ops, Content, Context, IntoValue, Recipe, SequenceElem, Styles, Unlabellable, Value,
};
...
impl Joiner {
    ...
    pub fn join(&mut self, other: Value) -> HintedStrResult<()> {
        if matches!(other, Value::None) {
            return Ok(());
        }
            ...
            match segment {
                Segment::Value(value) => {
                    if let Some(taken) = current.take() {
                        ...
                        debug_assert!(
                            self.is_display || matches!(taken, Value::Content(_))
                        );
                        current = Some(
